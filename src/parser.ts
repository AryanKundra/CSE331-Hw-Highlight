 ...
 * @param chars a list of characters which are parsed
...
if (chars === nil){
  ...
  if (next !== undefined){
    const [bef, highlight, aft] = next
    if (bef){
      return cons({color: 'white', text:bef }, cons(highlight, parseHighlights(aft)))
    } else{
      return cons(highlight, parseHighlights(aft))
    }
  }else{
